...
int obe_probe_device( obe_t *h, obe_input_t *input_device, obe_input_program_t *program )
{
    ...
    else if (input_device->input_type == INPUT_DEVICE_VEGA3311)
        input = vega3311_input;
#endif
...
#endif
#if defined(__APPLE__)
    else if (input_device->input_type == INPUT_DEVICE_AVFOUNDATION)
        input = avfoundation_input;
#endif
    else
    {
        fprintf(stderr, "Invalid input device, input = %d\n", input_device->input_type);
        return -1;
    }
    ...
    if( input_device->location )
        ...
        printf( "Probing device: Vega3311 input %i. ", input_device->card_idx);
#endif
    else if (input_device->input_type == INPUT_DEVICE_V210)
        printf( "Probing device: YUV card %i. ", input_device->card_idx);
#if defined(__APPLE__)
    else if (input_device->input_type == INPUT_DEVICE_AVFOUNDATION)
        printf( "Probing device: AVFoundation card %i. ", input_device->card_idx);
#endif
    else
...
#endif
#if HAVE_VEGA3311_CAP_TYPES_H
    else if (h->devices[0]->device_type == INPUT_DEVICE_VEGA3311)
        input = vega3311_input;
#endif
...
#endif
#if defined(__APPLE__)
    else if (h->devices[0]->device_type == INPUT_DEVICE_AVFOUNDATION)
        input = avfoundation_input;
#endif
    else
    {
    ...
    for( int i = 0; i < h->num_output_streams; i++ )
    {
        ...
        if (os->stream_action == STREAM_ENCODE )
        {
            ...
            else if (ostream->stream_format == VIDEO_HEVC_VEGA3301)
            {
                ...
                {
                    fprintf( stderr, "Couldn't create vega3301 encode thread\n" );
                    goto fail;
                }
            ...
            else if ((ostream->stream_format == VIDEO_HEVC_VEGA3311) || (ostream->stream_format == VIDEO_AVC_VEGA3311))
            {
                ...
                if (pthread_create(&h->encoders[h->num_encoders]->encoder_thread, NULL, vega_obe_encoder.start_encoder, (void*)vid_enc_params) < 0)
                {
                    fprintf( stderr, "Couldn't create vega3311 encode thread\n" );
                    goto fail;
                }
                ltnpthread_setname_np(h->encoders[h->num_encoders]->encoder_thread, "obe-vega-encoder");
            }
...
#endif
#if HAVE_VA_VA_H
            else if (ostream->stream_format == VIDEO_AVC_CPU_AVCODEC)
            {
                x264_param_t *x264_param = &ostream->avc_param;
                if( h->obe_system == OBE_SYSTEM_TYPE_LOWEST_LATENCY )
                {
